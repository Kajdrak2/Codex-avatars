...
> État du projet : la version `0.7.1` est publiée sur GitHub. Elle corrige le bouton **Démarrer avec Windows**, respecte l’état activé ou désactivé enregistré par Windows et regroupe automatiquement les anciennes entrées de démarrage en double. Elle inclut aussi le marketplace externe de Pets V2, la publication automatique après une CI protégée, le signalement public des Pets depuis l’application, la soumission directe via GitHub, la réconciliation continue des tâches multiples, des aperçus fiables pour les Pets propres au fork et le retour guidé décrits plus bas. L’installeur n’est pas encore signé.
